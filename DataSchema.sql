...
CREATE TABLE users (
    ...
    CHECK (
        (user_type = 'scientist' AND field_of_study IS NOT NULL) OR
        (user_type = 'manager' AND department IS NOT NULL)
    )
);
...
CREATE TABLE resources (
    ...
    CHECK (publication_year IS NULL OR (publication_year >= 1500 AND publication_year <= EXTRACT(YEAR FROM CURRENT_DATE))),
    -- Перевірка логіки
    CHECK (
        (resource_type = 'equipment' AND model_number IS NOT NULL) OR
        (resource_type = 'literature' AND author IS NOT NULL AND publication_year IS NOT NULL)
    )
);
...
CREATE TABLE online_orders (
    ...
    order_status VARCHAR(50) NOT NULL DEFAULT 'new', -- Виправлено: 'status' -> 'order_status'
...
CREATE TABLE scientific_ideas (
    ...
    FOREIGN KEY (discussion_id) REFERENCES online_discussions(discussion_id) ON DELETE CASCADE
);
...
CREATE TABLE comments (
    ...
    CHECK (
        (discussion_id IS NOT NULL AND idea_id IS NULL) OR
        (discussion_id IS NULL AND idea_id IS NOT NULL)
    )
);
